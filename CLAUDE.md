...
- **Viewing another user's pages ("view as"):** an admin browsing `/admin/[userId]/*` sees the *exact same* Today/Exercises/Stats page components the signed-in user would, read-only, rather than a separately maintained admin view. This is deliberate, so future features don't need a parallel admin UI:
  ...
  - The client sends the viewed user's id via an `x-view-as-user-id` header, toggled by a small module-level store (`src/lib/view-as.ts`) that `ViewAsProvider` (`src/components/admin/view-as-context.tsx`) sets on mount/unmount. `useViewAsUser()` reads it back for read-only UI gating (hide "+", edit, delete controls); `useAppPath()` rewrites an app-relative link so in-page navigation stays inside the admin view instead of jumping to the admin's own pages.
  - Routes under `/admin/[userId]/...` are thin re-exports of the real pages (e.g. `export { default } from "@/app/today/page"`) wrapped by `src/app/admin/[userId]/layout.tsx`, which mounts `ViewAsProvider`, shows the amber "viewing as" banner, and hosts the delete-user control. Adding a new top-level page later only needs one such re-export file to become admin-viewable — no duplicated UI.
- Deleting a user (`admin.deleteUser`) removes their `exercises`/`exercise_groups`/`profiles` rows (cascades `sets` and `exercise_group_members` via existing FKs) and then their Supabase Auth account; blocked for the admin's own account. The client gates the button (in the admin layout's banner) behind typing the exact username in a confirmation modal — irreversible, so no soft-delete/undo.
