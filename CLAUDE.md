...
Every user sees the same standard exercise list by default — `exercises.user_id IS NULL` — rather than each user getting their own duplicated copy. A standard exercise's name, description, and tracked fields are **read-only for everyone** (`exercise.update` only ever matches rows where `userId = ctx.userId`, so it 404s on a standard exercise) — that's deliberate, not a missing feature: an earlier version let editing a standard exercise "fork" a personal copy, but a small edit (even a typo) then silently produced a second, confusingly-similar exercise with its own history, with no obvious way back. Locking the definition avoids that entirely, while `exercise.updateGroups` still lets each user file a standard exercise into their own groups freely — organization is personal, the definition (and therefore comparability with other users) isn't. A genuinely different exercise is just a new one, created from scratch (fully owned, fully editable, deletable). `ExerciseCard`/`ExerciseSummaryRow` show a "Custom" badge for any exercise with a non-null `userId` so it's visually distinct from a standard one.
